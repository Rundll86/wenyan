...
export class VM {
  ...
  // 执行整个程序
  public execute(program: ProgramNode): any {
    let result: any = null;
  ...
  // 执行单个节点
  public executeNode(node: Node): any {
    switch (node.type) {
  ...
  // 执行导入声明
  private executeImportDeclaration(node: ImportDeclarationNode): any {
    ...
    // 将符号导入到当前环境
    const importedSymbols: Record<string, any> = {};
    
    for (const symbol of symbols) {
      if (symbol in module) {
        this.environment.functions[symbol] = module[symbol];
        importedSymbols[symbol] = module[symbol];
      } else {
  ...
  // 执行函数调用
  private executeFunctionCall(node: FunctionCallNode): any {
  ...
  // 执行表达式
  private executeExpression(node: ExpressionNode): any {
  ...
  // 解析标识符
  private resolveIdentifier(node: IdentifierNode): any {
    const { name } = node;
